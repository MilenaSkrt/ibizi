...
class UserAuthApp(QMainWindow):
    ...
    def ensure_admin_exists(self):
        ...
        if ADMIN_USERNAME not in users:
            ...
            if ok and admin_password:
                ...
                QMessageBox.information(self, 'УРА', 'Администратор создан!')
            else:
                QMessageBox.critical(self, 'ОЙ', 'Пароль администратора обязателен! Приложение закроется.')
                sys.exit()
    ...
    def save_password_rules(self):
        ...
        self.save_users(users)
        QMessageBox.information(self, 'УРА', 'Правила пароля сохранены!')
    ...
    def login(self):
        ...
        if username not in users:
            QMessageBox.warning(self, 'ОЙ', 'Пользователь не найден!')
            return

        if users[username]['blocked']:
            QMessageBox.warning(self, 'ОЙ', 'Ваш аккаунт заблокирован!')
            return

        if users[username]['password'] == self.hash_password(password):
            is_admin = users[username]['admin']
            self.update_admin_controls(is_admin)
            QMessageBox.information(self, 'УРА', 'Вход выполнен!')
        else:
            QMessageBox.warning(self, 'ОЙ', 'Неверный пароль!')

    def register_user(self):
        QMessageBox.information(self, 'Информация', 'Регистрация нового пользователя пока не реализована.')
    ...
    def toggle_user_block(self, block):
        ...
        if username in users and username != ADMIN_USERNAME:
            ...
            QMessageBox.information(self, 'УРА', f'Пользователь {username} {status}!')
